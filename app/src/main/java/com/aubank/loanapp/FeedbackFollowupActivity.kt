...
class FeedbackFollowupActivity : ComponentActivity(), LocationListener {

    private lateinit var fusedLocationClient: FusedLocationProviderClient
    private var currentPhotoPath: String? = null
    val viewModel = FeedbackFollowupViewModel(UserRepository())
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.approvalFeedbackData = intent.getParcelableExtra(Constants.PENDING_APPROVAL_FEEDBACK_DATA)!!
        viewModel.feedbackData.value!!.VisitDoneId = intent.getIntExtra(Constants.VISIT_DONE_ID, 0)
        viewModel.feedbackData.value!!.VisitType = "Follow up"
        ...
        { event ->
            when (event) {
                is FeedbackFollowupViewModel.PostFeedbackDataApiState.Success -> {
                    viewModel.isLoading = false
                    Toast.makeText(this, "Feedback submitted", Toast.LENGTH_LONG).show()
                    finish()
                }

                is FeedbackFollowupViewModel.PostFeedbackDataApiState.Error -> {
                    viewModel.isLoading = false
                    Toast.makeText(this, event.exception.toString(), Toast.LENGTH_LONG).show()
                }

                is FeedbackFollowupViewModel.PostFeedbackDataApiState.Loading -> {
...
@Composable
private fun FeedbackFollowupScreen(
    app: PendingApp,
    viewModel: FeedbackFollowupViewModel,
...
private fun FeedbackForm(
    ...
    {
        viewModel.feedbackData.value!!.VisitDate = DateTimeFormatter.formatVisitDateTimeFormat(
            LocalDateTime.now())

        LabelLineValue(label = "Visit Date", value = DateTimeFormatter.formatDate(LocalDateTime.now()))
        Spacer(modifier = Modifier.height(16.dp))
        LabelLineValue(label = "Visit Type", value = viewModel.feedbackData.value!!.VisitType)
        Spacer(modifier = Modifier.height(16.dp))
        LabelLineValue(label = "Name of Borrower", value = app.borrowerName)
        Spacer(modifier = Modifier.height(16.dp))
        LabelLineValue(label = "Visit Done", value = masterData.visitDones.find { visitDone: VisitDone ->  visitDone.visitDoneId == feedbackData.VisitDoneId}.toString())
        Spacer(modifier = Modifier.height(16.dp))
        LabelLineValue(label = "Borrower/Family Currently Living at Given Address", value = if (viewModel.approvalFeedbackData.borrowerLivingCurrentAddress) "Yes" else "No")
        if (!feedbackData.BorrowerLivingCurrentAddress) {
            Spacer(modifier = Modifier.height(16.dp))
            LabelLineValue(label = "If Not Then New Address of Borrower", value = viewModel.approvalFeedbackData.newAddressOfBorrower ?: "")
        }

        Spacer(modifier = Modifier.height(16.dp))
        LabelLineValue(label = "Is This Case Workable or Non-Workable(Last Submitted)",
            value = if (viewModel.approvalFeedbackData.workableNonWorkable) "Workable" else "Non Workable")

        Spacer(modifier = Modifier.height(16.dp))
        LabelLineValue(label = "Reasons for Workable or Non-Workable (Last Submitted)",
            value = viewModel.approvalFeedbackData.reasonforWorkable ?: "")
        ...
        if (viewModel.approvalFeedbackData.litigationId > 0) {
            Spacer(modifier = Modifier.height(16.dp))
            LabelLineValue(
                label = "New litigation Required other Then Running(Last Submitted)",
                value = masterData.litigations.find { litigation: Litigation ->  litigation.litigationId == viewModel.approvalFeedbackData.litigationId}.toString(),)

            Spacer(modifier = Modifier.height(16.dp))
            LabelLineValue(
                label = "Why New Litigation Required(Last Submitted)",
                value = viewModel.approvalFeedbackData.newLitigationReuired ?: "",
            )
        }
        ...
        Spacer(modifier = Modifier.height(16.dp))
        LabelLineValue(
            label = "Any Settlement Proposal or PTP with Date(Last Submitted)",
            value = viewModel.approvalFeedbackData.anySettlementProposal ?: "",
        )
